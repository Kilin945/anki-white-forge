...
- **TestCardsSection = 手動測 dialog 用的測試卡工具（開發輔助）**。建只有 `Front`+`Association` 的裸卡（Front 用純字母假詞 `zztest…`，因 `_looks_english` 擋數字）→ 它們會出現在 Complete Missing Cards 讓你勾選測試（如 Remove Finished）。全部打 tag `whiteforge_test`，Clean 靠 tag 一鍵刪。同步、直接動 `mw.col`（`new_note`/`add_note`/`find_notes`/`remove_notes`），無 worker。**與 CLI `make_test_cards.py` 是同一工具的兩份實作**（addon 不能 import core → tag 與假詞清單各存一份，標 KEEP-IN-SYNC；同 tag 故兩邊建的可互相清）。純邏輯 `_clamp_test_count`（Count 欄解析/夾限）抽出來給 pytest 測（`test_test_cards.py`）。
...
- **批次類視窗（⌘A/⌘S/⌘D/⌘F）一律非阻塞開啟，走 `_show_nonmodal()`**（`aqt.dialogs.open`），**不要用 `exec()`**（application-modal 會鎖整個 Anki）。**`exec()` 除了「擋住使用者」還隱性提供三件事，非阻塞化必須自己補回**（第一版只換 show() 沒補，code review 抓出 6 個洞）：
  ...
  - ⌘S 收尾後出現的 **`Remove Finished` 只拿掉「重讀欄位確認補齊」的列**（純函式 `_finished_ids`，卡片已被別處刪掉也算 → 既補不了也不用補）。**不看勾選**：開窗預設全選，綁勾選等於「按一下清空整個清單」，連沒補完的都看不到了。**不靠 `card_done`**：它的語意是「這張處理完了」非「補齊了」，欄位生失敗只 emit step `warn` 不中斷 → 半成品照樣發 card_done（同上面舊 Refill 的坑）。沒補完的列留著且**維持勾選**，額度回來直接再按 Complete Selected 續跑。回歸保護在 `check_qt_runtime.py` 的「Remove Finished 只算補齊的卡」一項（真的讓一張的翻譯回空字串）
...
- **pytest 全綠不代表 Qt 沒事** <!-- @assert:path check_qt_compat.py --> <!-- @assert:path check_qt_runtime.py -->：`conftest.py` 的假 aqt 把每個 Qt 類別換成 `_Any`（`AddWordDialog.__mro__` 裡根本沒有 `QDialog`）→ 那批測試驗的是純邏輯，對「這一版 Qt 還能不能用」**零覆蓋**。Anki / Qt 升級後要跑兩支（都 offscreen、不需要 Anki 開著、不碰真卡片）：
  - `uv run python check_qt_compat.py` —— **建得起來嗎**：import、21 個 Qt 名稱、四個對話框建構、快捷鍵綁定、`aqt.dialogs` 的登記／單例／`closeAll()`（11 項）
  - `uv run python check_qt_runtime.py` —— **跑得起來嗎**：QThread 跑完整批、`pyqtSignal` 跨執行緒送達 GUI thread、批次互斥擋下五個入口、跑批中關窗三條路徑（`done()` 守門／`_force_close()`／`closeWithCallback()`）、QMessageBox 三個自訂 ButtonRole、QTest 真實按鍵、64-bit note id 無損、⌘F 四個 section 的實際動作（32 項）
  兩支都把 `Anki.app/Contents/Resources/app_packages` 加進 `sys.path`（**只能加這個**，`Resources/app` 下另一個 `anki` 套件會遮蔽真正的）。覆蓋面只到「想得到的破法」為止——遇到新的升級踩雷就往裡面加一條，不加它就永遠停在當時的覆蓋面
